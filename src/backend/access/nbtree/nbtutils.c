...
{
	if (TupleDescAttr(rel->rd_att, 0)->atttypid == INT4OID)
		return _bt_abbreviate_int4;
	else if (TupleDescAttr(rel->rd_att, 0)->atttypid == TEXTOID &&
			 TupleDescAttr(rel->rd_att, 0)->attcollation == DEFAULT_COLLATION_OID)
