 ...
 *
 * * All FdwXact fields except for indoubt, inprocessing, and status are
 *   protected by FdwXactLock.  These three fields are protected by its mutex.
 * * Insertion, deleteion and iteration must hold FdwXactLock.
 * * Backend process can set held_by of an FdwXact entry to own the FdwXact
 *   entry, which prevents the entry from updated and removed by concurrent
 *   processes.
 * * Resolver process can set inprocessing to true, which also prevents it from
 *   updated removed by concurrent processes.  Therefore, an FdwXact entry can
 *   be owned by a backend process while being processed a resolver process.
 * * A process who is going to process foreign transaction needs to hold its
 *   FdwXact entry in advance by either the above ways.
 *
 * The held_by is also used to check if the FdwXact entry is still held before
 * marking the foreign transaction as in-doubt in failure cases. The inprocessing
 * must be cleared when the resolver process exits.
 *
 * RECOVERY
 *
...
{
	/*
	 * After preparing the local transaction, we forget all participants but
	 * not mark them as in-doubt because these transactions needs to be
	 * resolved when COMMIT or ROLLBACK PREPARED.
	 */
...
FdwXactWaitForResolution(TransactionId wait_xid)
{
	...
	for (;;)
	{
		...
		{
			/* All foreign transactinos are resolved, free the list */
			list_free(FdwXactParticipants);
			FdwXactParticipants = NIL;
			break;
		}
...
AtEOXact_FdwXact(bool is_commit)
{
	...
	if (!is_commit)
	{
		foreach(lc, FdwXactParticipants)
		{
			 ...
			 */
			Assert(fdwxact->held_by == MyBackendId);
		...
		if (FdwXactParticipants != NIL)
		{
			...
			FdwXactWaitForResolution(FdwXactLocalXid);
		}
	}
 ...
 * list. If mark_indoubt is true, we also mark the participants as in-doubt.
 ...
 * values for respective datatypes.  If 'hold' is true, we held the matched FdwXact
 * by MyBackendId.
 */
static int
