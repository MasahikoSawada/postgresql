...
/*
 * Stop the fdwxact resolver running on the current database
 */
Datum
pg_stop_fdwxact_resolver(PG_FUNCTION_ARGS)
{
	int	pid = InvalidPid;
	int i;
	...
	for (i = 0; i < max_foreign_xact_resolvers; i++)
	{
		...
		if (resolver->dbid == MyDatabaseId)
			pid = resolver->pid;
	}
	LWLockRelease(FdwXactResolverLock);

	if (pid == InvalidPid)
		elog(ERROR, "could not find foreign transaction resolver running on database \"%s\"",
...
/*
 * Stop the all running fdwxact resolvers
 */
Datum
pg_stop_all_fdwxact_resolvers(PG_FUNCTION_ARGS)
{
	List		*pids = NIL;
	ListCell	*lc;
	int			i;
	...
	/* Find all running resolver processes */
	LWLockAcquire(FdwXactResolverLock, LW_SHARED);
	for (i = 0; i < max_foreign_xact_resolvers; i++)
	{
		FdwXactResolver	*resolver = &FdwXactRslvCtl->resolvers[i];
		...
		if (resolver->pid != InvalidPid)
			pids = lappend_int(pids, resolver->pid);
	}
	LWLockRelease(FdwXactResolverLock);

	foreach (lc, pids)
	{
		elog(NOTICE, "pid %d", lfirst_int(lc));
		//kill(lfirst_int(lc), SIGTERM);
	}
