...
#ifdef RT_SHMEM
RT_SCOPE	RT_RADIX_TREE *RT_CREATE(MemoryContext ctx, size_t max_bytes, dsa_area *dsa,
									 int tranche_id);
...
#else
RT_SCOPE	RT_RADIX_TREE *RT_CREATE(MemoryContext ctx, size_t max_bytes);
#endif
...
RT_SCOPE bool
#ifdef RT_VARLEN_VALUE
RT_SET(RT_RADIX_TREE * tree, uint64 key, RT_VALUE_TYPE * value_p, size_t value_sz)
#else
...
#ifdef RT_SHMEM
RT_CREATE(MemoryContext ctx, size_t max_bytes, dsa_area *dsa, int tranche_id)
#else
RT_CREATE(MemoryContext ctx, size_t max_bytes)
#endif
{
	RT_RADIX_TREE *tree;
