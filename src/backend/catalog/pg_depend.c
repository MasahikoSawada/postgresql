 ...
 *
 * This is the oposite of recordDependencyOn.
 * Like the above deleteDependencyRecordsForClass, this is useful when
 * change or property changes dependency requirements.
 */
long
deleteDependencyOn(const ObjectAddress *depender,
				   const ObjectAddress *referenced, DependencyType deptype)
{
	long		count = 0;
	Relation	depRel;
	ScanKeyData key[2];
	...
	while (HeapTupleIsValid(tup = systable_getnext(scan)))
	{
		...
		if (depform->refclassid == referenced->classId &&
			depform->refobjid == referenced->objectId &&
