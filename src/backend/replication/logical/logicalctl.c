...
EnableLogicalDecoding(void)
{
	 ...
	 * long as they valid XIDs. These transactions will appear in the
	 ...
	 * read-only transactions, to finish before enabling logical decoding.
	 * However, such a requirement would force the slot creation to wait for a
	 * potentially very long time due to long-running read queries, which is
