...
static Buffer _bt2_split(Relation rel, Buffer buf, Buffer cbuf,
		  OffsetNumber firstright, OffsetNumber newitemoff, Size newitemsz,
		  IndexTuple newitem, int32* newabbrev, bool newitemonleft);
...
static bool _bt2_pgaddtup_internal(Page page, Size itemsize, IndexTuple itup,
			 OffsetNumber itup_off, uint16 abbrkey);
...
_bt2_split(Relation rel ,Buffer buf, Buffer cbuf, OffsetNumber firstright,
		  OffsetNumber newitemoff, Size newitemsz, IndexTuple newitem, int32* newabbrev,
		  bool newitemonleft)
{
	Page			page;
	BTPageOpaque	opaque;
...
*/
	...
	if (!P_ISLEAF(opaque))
		return _bt2_split_internal(rel, buf, cbuf, firstright, newitemoff, newitemsz,
						   newitem, newabbrev, newitemonleft);
	else
		  ...
		  bool newitemonleft)
{
	...
	{
		itemid = PageGetItemIdWithAbbrKey(origpage, P_HIKEY);
		itemsz = ItemIdGetLength(itemid);
		item = (IndexTuple) PageGetItem(origpage, itemid);
		abbrkey = ItemIdGetAbbrKey(itemid);
		if (PageAddItemWithAbbrKey(rightpage, (Item) item, itemsz, rightoff,
	...
	if (!newitemonleft && newitemoff == firstright)
	{
		...
		itemsz = newitemsz;
		item = newitem;
		abbrkey = int32AbbrevConvert(*newabbrev);
	}
	else
	{
		/* existing item at firstright will become first on right page */
		itemid = PageGetItemIdWithAbbrKey(origpage, firstright);
		itemsz = ItemIdGetLength(itemid);
		item = (IndexTuple) PageGetItem(origpage, itemid);
		abbrkey = ItemIdGetAbbrKey(itemid);
	}
	if (PageAddItemWithAbbrKey(leftpage, (Item) item, itemsz, leftoff,
							   false, false, abbrkey) == InvalidOffsetNumber)
	{
		memset(rightpage, 0, BufferGetPageSize(rbuf));
	...
	{
		itemid = PageGetItemIdWithAbbrKey(origpage, i);
		itemsz = ItemIdGetLength(itemid);
		item = (IndexTuple) PageGetItem(origpage, itemid);
		abbrkey = ItemIdGetAbbrKey(itemid);
				  ...
				  bool is_only)
{
	...
	else
	{
		...
		Buffer		pbuf;
		int32		abbrkey;
		BTPageOpaque lopaque = (BTPageOpaque) PageGetSpecialPointer(page);
		...
		else
		{
			/* get high key from left page == lowest key on new right page */
			ritem = (IndexTuple) PageGetItem(page,
											 PageGetItemIdWithAbbrKey(page, P_HIKEY));
			...
			abbrkey = ItemIdGetAbbrKey(PageGetItemIdWithAbbrKey(page, P_HIKEY));
			/*
			 * Now we can unlock the right child. The left child will be unlocked
			 * by _bt_insertonpg().
			 */
...
_bt2_newroot_internal(Relation rel, Buffer lbuf, Buffer rbuf)
{
	 ...
	 */
	itemid = PageGetItemIdWithAbbrKey(lpage, P_HIKEY);
