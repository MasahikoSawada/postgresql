...
#define FdwXactIsResolving(status) \
	(status == FDW_XACT_STATUS_PREPARING || \
	 status == FDW_XACT_STATUS_COMMITTING || \
	 status == FDW_XACT_STATUS_ABORTING)
...
static FdwXactResolveState *create_fdw_xact_state(void);
 ...
 * participants chain is used to access all participants involved in the distributed
 * transaction. If any one of them fails to prepare, we change over aborts.
 */
static void
FdwXactPrepareForeignTransactions(void)
{
	...
	foreach(lcell, FdwXactParticipants)
	{
		 ...
		 * when connectivity is restored or after crash recovery.
		 *
		...
		state->fdwxact_id = pstrdup(fdw_part->fdw_xact_id);
		 ...
		 *
		 * During abort procesing, we might try to resolve a never-preapred
		 * transaction, and get an error. This is fine as long as the FDW provides
		 * us unique prepared transaction identifiers.
		 */
		...
		PG_TRY();
		{
			fdw_part->prepare_foreign_xact(state);
		}
		PG_CATCH();
		{
...
FdwXactInsertFdwXactEntry(TransactionId xid, FdwXactParticipant *fdw_part)
{
	...
	fxact->status = FDW_XACT_STATUS_INITIAL;
				...
				Oid umid, char *fdw_xact_id)
{
	...
	FdwXactCtl->fdwXacts[FdwXactCtl->numFdwXacts++] = fxact;
...
remove_fdw_xact(FdwXact fdw_xact)
{
	...
	{
		if (FdwXactCtl->fdwXacts[cnt] == fdw_xact)
			break;
	}
	...
	FdwXactCtl->fdwXacts[cnt] = FdwXactCtl->fdwXacts[FdwXactCtl->numFdwXacts];
	...
	/* Reset informations */
	fdw_xact->status = FDW_XACT_STATUS_INVALID;
...
FdwXactWaitToBeResolved(TransactionId wait_xid, bool is_commit)
{
	...
	/* Also, exit if the transaction itself has no foreign transaction participants */
	...
	else if
	{
		 ...
		 * part of a prepared transaction, or when we resolve the PREPARE'd
		 * distributed transaction after crash.
		 */
 ...
 * AtPrepare_FdwXacts
 *
 * If there are foreign servers involved in the transaction, we prepare them.
 *
 ...
 */
bool
FdwXactResolveInDoubtTransactions(Oid dbid)
{
	List		*indoubt_txs;
	int			n_resolved = 0;
	int			i;
	...
	/* Get all in-doubt foreign transactions */
	indoubt_txs = get_fdw_xacts(dbid, InvalidTransactionId, InvalidOid,
								true, true);
...
static bool
FdwXactDetermineTransactionFate(FdwXact fdwxact)
{
...
FdwXactResolveForeignTransaction(FdwXactResolveState *state, FdwXact fdwxact,
								 bool is_commit, FdwXactStatus fallback_status)
{
	ForeignServer		*server;
	ForeignDataWrapper	*fdw;
...
fdw_xact_exists(TransactionId xid, Oid dbid, Oid serverid, Oid userid)
{
	...
	/*
	 * @@@ should include in-doubts?
	 */
	fdw_xact_list = get_fdw_xacts(dbid, xid, serverid, userid, false, true);
			  ...
			  bool including_indoubts, bool need_lock)
{
	...
	for (i = 0; i < FdwXactCtl->numFdwXacts; i++)
	{
		FdwXact	fdw_xact = FdwXactCtl->fdwXacts[i];
		bool	matches = true;

		if (FdwXactIsResolving(fdw_xact->status))
			continue;
		...
		/* xid */
		if (TransactionIdIsValid(xid) && xid != fdw_xact->local_xid)
			matches = false;
		...
		if (!including_indoubts && fdw_xact->status != FDW_XACT_STATUS_IN_DOUBT)
			matches = false;
...
generate_fdw_xact_identifier(TransactionId xid, Oid serverid, Oid userid)
{
	...
	snprintf(fdw_xact_id, FDW_XACT_ID_MAX_LEN, "fx_%ld_%u_%d_%d",
			 Abs(random()), xid, serverid, userid);
...
CheckPointFdwXacts(XLogRecPtr redo_horizon)
{
	...
	for (cnt = 0; cnt < FdwXactCtl->numFdwXacts; cnt++)
	{
		FdwXact		fxact = FdwXactCtl->fdwXacts[cnt];
...
{
#define PG_PREPARED_FDW_XACTS_COLS	6
	...
	if (SRF_IS_FIRSTCALL())
	{
		...
		tupdesc = CreateTemplateTupleDesc(PG_PREPARED_FDW_XACTS_COLS);
	...
	while (status->cur_xact < status->num_xacts)
	{
		...
		Datum		values[PG_PREPARED_FDW_XACTS_COLS];
		bool		nulls[PG_PREPARED_FDW_XACTS_COLS];
		HeapTuple	tuple;
		Datum		result;
...
pg_resolve_fdw_xact(PG_FUNCTION_ARGS)
{
	...
	fdwxact = get_one_fdw_xact(MyDatabaseId, xid, serverid, userid, true, false);
