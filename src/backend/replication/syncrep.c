...
SyncRepGetQuorumRecPtr(SyncInfoNode * node, List **lsnlist, bool priority_group)
{
	...
	if (node->gtype == GNODE_NAME)
	{
		 ...
		 * select higher. XXX : Is it right way?
		 */
...
#ifdef DEBUG_QUORUM
				/* debug print */
				elog(WARNING, "NAME:[%d]---- [%s] : write : %X/%X, flush : %X/%X",
					 MyProcPid,
					 node->name,
...
print_structure(SyncInfoNode * expr, int level)
{
	...
	{
		elog(WARNING, "[GROUP]: %s quorum = %d name = %s", blank, expr->count, expr->name);
		level++;
		if (expr->group)
...
add_node(SyncInfoState * state, SyncInfoNodeType ntype)
{
	...
	switch (pstate->state_name)
	{
		...
		case SYNC_INFO_GROUP_INFO:
			if (++(state->array_element_count) == 0)
				state->array_first_node = new_node;
			else if (state->array_element_count > 0)
...
pushState(SyncInfoParseState ** pstate, SyncParseStateName state)
{
	...
	if (state != SYNC_INFO_MAIN_OBJECT_START && state < (*pstate)->state_name)
		elog(ERROR, "Incorrect Order in JSON forat of pg_syncinfo.conf");

	ns->state_name = state;
	ereport(WARNING, (errmsg("STATE in: %d", state)));
	ns->next = *pstate;
	return ns;
}
...
popState(SyncInfoParseState ** pstate)
{
	...
	ereport(WARNING, (errmsg("STATE out: %d", (*pstate)->state_name)));
	pfree(*pstate);
	return ps;
}
...
sync_info_array_end(void *istate)
{
	...
	if (state->parse_state->state_name == SYNC_INFO_GROUP_INFO)
		if (!populate_group(SyncRepStandbyInfo, state->group_name, state->array_first_node, false))
			ereport(LOG,
					(errcode(ERRCODE_INVALID_OBJECT_DEFINITION),
					 errmsg("\"%s\": group \"%s\" defined but not used",
							SyncFileName, state->group_name)));

	if (state->parse_state->state_name == SYNC_INFO_NODES){
		state->cur_node = SyncRepStandbyInfo;
		SyncRepStandbyInfo->group = state->array_first_node;
}

	state->array_element_count = -1;
...
/*			ereport(WARNING, (errmsg(">>>>%s",fname))); */
	...
	switch (pstate->state_name)
	{
		case SYNC_INFO_MAIN_OBJECT_START:
			...
			else if (strcmp(fname, "groups") == 0)
			{
				...
				elog(ERROR, "Wrong Key %s", fname);
			break;

		case SYNC_INFO_GROUPS:
			{
				state->group_name = fname;
				state->parse_state = pushState(&(state->parse_state), SYNC_INFO_GROUP_INFO);
				break;
			}
...
sync_info_scalar(void *istate, char *token, JsonTokenType tokentype)
{
	...
	ereport(WARNING, (errmsg(">>tks %s", token)));
	Assert(token != NULL);
	switch (tokentype)
	{
		...
		case JSON_TOKEN_STRING:
			{
				...
				if (pstate->state_name == SYNC_INFO_NODES || pstate->state_name == SYNC_INFO_GROUP_INFO)
...
load_syncinfo()
{
	...
	if (strcmp(SyncRepStandbyNames, SYNC_FILENAME) == 0)
	{
		...
		elog(LOG, ">>%s", text_to_cstring(sync_info));
