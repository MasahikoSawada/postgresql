...
BufFileLoadBuffer(BufFile *file)
{
	...
	/*
	 * Only whole multiple of ENCRYPTION_BLOCK_SIZE can be encrypted / decrypted,
	 * see comments in BufFileDumpBuffer().
	 */
	Assert((file->common.curOffset % BLCKSZ == 0 &&
			file->common.curOffset % ENCRYPTION_BLOCK_SIZE == 0) ||
		   !TransparentEncryptionEnabled());
	...
	if (TransparentEncryptionEnabled() && file->common.nbytes > 0)
	{
...
BufFileDumpBuffer(BufFile *file)
{
	...
	while (wpos < file->common.nbytes)
	{
		...
		else
		{
			 ...
			 * ENCRYPTION_BLOCK_SIZE. We meet this condition simply by encrypting
			 * the whole buffer.
			 *
		...
		if (bytestowrite <= 0 ||
			(TransparentEncryptionEnabled() && bytestowrite != BLCKSZ))
...
BufFileTweak(char *tweak, BufFileCommon *file, bool is_transient)
{
	...
	if (!is_transient)
	{
		...
		StaticAssertStmt(sizeof(pid) + sizeof(number) + sizeof(block) <=
						 ENCRYPTION_TWEAK_SIZE,
						 "tweak components do not fit into ENCRYPTION_TWEAK_SIZE");
		 ...
		 * shared one. Since there's no more room within ENCRYPTION_TWEAK_SIZE, should we
...
#define BUF_FILE_PATH_HASH_LEN	8
		 ...
		 * tweak value, however usual hashes do not fit into ENCRYPTION_TWEAK_SIZE. The
		...
		StaticAssertStmt(BUF_FILE_PATH_HASH_LEN + sizeof(block) <= ENCRYPTION_TWEAK_SIZE,
						 "tweak components do not fit into ENCRYPTION_TWEAK_SIZE");
				  ...
				  bool is_transient)
{
	...
	while (size > 0)
	{
		if (file->pos >= file->nbytes)
		{
			 ...
			 * requirement too.
			 */
			Assert(file->curOffset % BLCKSZ == 0 || !TransparentEncryptionEnabled());

			file->nbytes = 0;
			/* Try to load more data into buffer. */
			if (!TransparentEncryptionEnabled() || file->pos % BLCKSZ == 0)
			{
	...
	while (size > 0)
	{
		if (file->pos >= BLCKSZ)
		{
			...
			else
			{
				 ...
				 * to do about TransparentEncryptionEnabled(), except for zeroing the buffer.
				 */
