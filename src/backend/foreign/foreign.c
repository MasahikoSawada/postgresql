...
GetFdwRoutine(Oid fdwhandler)
{
	...
	if ((routine->CommitForeignTransaction &&
		...
		(!routine->CommitForeignTransaction &&
		 routine->RollbackForeignTransaction))
		elog(ERROR,
			 "foreign-data-wrapper supports only either commit or rollback routine");

	if (routine->PrepareForeignTransaction &&
		(!routine->CommitForeignTransaction ||
		 !routine->RollbackForeignTransaction))
		elog(ERROR,
			 "foreign-data wrapper supports prepare routine but not support commit and rollback routines.");
